...
#mono_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=25, gamma=0.1)
multi_lr_scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[15,25,40,60], gamma=0.3)

def train(epoch):
    #mono_lr_scheduler.step()
...
def validations():
    model.eval()
    validation_loss = 0
    mapping = {0 : 'Groove_billed_Ani', 1 : 'Brewer_Blackbird', 2 : 'Red_winged_Blackbird', 3 : 'Rusty_Blackbird', 
              4 : 'Yellow_headed_Blackbird', 5 : 'Bobolink', 6 : 'Indigo_Bunting', 7 : 'Lazuli_Bunting', 
              8 : 'Painted_Bunting', 9 : 'Gray_Catbird', 10 : 'Yellow_breasted_Chat', 11 : 'Eastern_Towhee', 12 : 'Brandt_Cormorant', 
              13 : 'Bronzed_Cowbird', 14 : 'Brown_Creeper', 15 : 'American_Crow', 16 : 'Fish_Crow', 
              17 : 'Black_billed_Cuckoo', 18 : 'Yellow_billed_Cuckoo', 19 : 'Gray_crowned_Rosy_Finch'}
    ...
    for bird in range(20):
        if count[bird] != 0:
            print('\nValidation set for ' + mapping[bird] + ' : Accuracy: {}/{} ({:.0f}%)\n'.format(
                correct[bird], count[bird],
                100. * correct[bird] / count[bird]))
            
    validation_loss /= len(val_loader.dataset)
    print('\nValidation set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
        validation_loss, correct_all, len(val_loader.dataset),
        100. * correct_all / len(val_loader.dataset)))
